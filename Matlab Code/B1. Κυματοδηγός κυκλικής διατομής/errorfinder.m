function [error, percent, Fields, cutofFreqTheory] = ...
errorfinder(eigVec, eigVal, numModes, modeType, radius)



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%       Calculate the theoretical cut off frequencies of modes
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


KcPredict = sqrt(eigVal);
KcTheory = zeros(numModes + 1, numModes);

for n = 0 : numModes
    for m = 1 : numModes
        
        if isequal(modeType, 'TE') 
            KcTheory(n+1, m) = besselderivzero(n, m) / radius;
            
        elseif isequal(modeType, 'TM')
            KcTheory(n+1, m) = besselzero(n, m, 1) / radius;
        end
    end
end
KcTheory = sort(KcTheory(:));
KcTheory = KcTheory(1:numModes);
cutofFreqTheory =  physconst('LightSpeed') / (2 * pi) * KcTheory;
cutofFreqPredict = physconst('LightSpeed') / (2 * pi) * KcPredict;


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%               Use the theoretical calculations to choose 
%                  the correct modes from the Rogue ones
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


error = zeros(numModes, 1);
percent = zeros(numModes, 1);

Fields = zeros(size(eigVec, 1), numModes);
cutofFreq = zeros(numModes, 1);

for mode = 1 : numModes
    
    absError = abs(cutofFreqPredict - cutofFreqTheory(mode));       
    index = (absError > error( max(mode-1, 1)));                    % error must be an increasing function
    [minError, minIdx] = min(absError(index));                      % choose the mode with min error
    error(mode) = minError;                                         % error of aproximation
    percent(mode) = 100 * minError / cutofFreqTheory(mode);         % percentege of error
    
    Fields(:, mode) = eigVec(:, minIdx);
    cutofFreq(mode) = cutofFreqPredict(minIdx);
    
    
    % Find modes that are similar and discard them
    tol = 1e-3;           % tol = 5e-3; for refinements <= 1 && tol = 1e-3; for refinements >= 2
    
    % find the percentage of similarity between the detected cutoff frequency and the rest
    similarError = abs(cutofFreqPredict - cutofFreqPredict(minIdx)) / cutofFreqPredict(minIdx);  
    similarIdx = similarError <= tol;     % find the indicies that are similar within a tolerance factor
    cutofFreqPredict(similarIdx) = [];    % discard those values
    eigVec(:, similarIdx) = [];           % discard their coresponding modes
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%           Functions from the Internet to calculate Bessel's zeros
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
